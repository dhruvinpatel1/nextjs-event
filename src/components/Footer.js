            ...
            <div className='w-full'>
              {
                footer.middle_Footer.left.map((label, index) => (
                  <div key={index} className='text-sm font-semibold text-[#767171] py-[13px] px-[20px] uppercase flex justify-end'>
                    <Link href={`https://thebrideshow.com${label.link}`}>{label.label}</Link>
                  </div>
                ))
              }
            </div>
          ...
          <ul className='flex justify-center '>
            {
              footer.bottom_Footer.map((label, index) => (
                <li key={index} className='inline-block relative'><Link href={label.link} className="py-[13px] px-[20px] text-xs font-semibold text-[#767171] hover:after:content-[''] hover:after:absolute hover:after:bottom-[-6px] hover:after:bg-[#767171] hover:after:w-full hover:after:h-[3px] hover:after:left-0">{label.label}</Link></li>
              ))
            }
          </ul>
        </div>
